		...
		<div onClick={() => navigate(`/place/${place.id}`)}
			...
			<div className="aspect-square w-full overflow-hidden">
				<img
					src={place.image}
					alt={place.name}
					className="h-full w-full p-3 object-cover transition-transform duration-300 group-hover:scale-105"
				/>
			</div>
			<div className="flex items-center justify-between h-10 px-3 py-2.5">
			...
			<p className="text-xs text-center py-2 text-[#111a2e]/50">{place.location}</p>
		</div>
	);
}
